...
이 대장은 **기계가 채우는 칸과 사람이 채우는 칸이 섞여 있다.** 갈라두지 않으면
재수입 때 사람이 채운 값이 지워진다.

    기계 소유   platform · service · name · native_id · account · region
                status · attrs
                -> 수집이 최신으로 덮어쓴다. 손으로 고쳐도 다음 수입에 되돌아간다

    사람 소유   tenant(소유 조직) · monthly_cost · description
                -> 수집이 알 수 없는 값이다. 변환기가 **아예 내보내지 않으므로**
                   재수입해도 건드려지지 않는다

    경계        environment
                -> 태그(env/environment/stage/tier)에 있으면 기계가 채우고,
                   없으면 사람 몫이다. 값이 있을 때만 내보낸다

규칙은 단순하다. **빈 값으로 내보내면 지워지고, 아예 안 내보내면 남는다.**
실측으로 확인한 동작이다.

### 그래서 재수입에는 JSON 을 쓴다

CSV 는 칸이 고정이라 행마다 뺄 수 없다. 변환기가 전 행이 빈 칸은 통째로 빼주지만,
**일부 행만 비어 있으면 그 행의 값은 지워진다.** JSON 은 행마다 키를 빼므로
그런 일이 없다.

    처음 채울 때   CSV 도 무방
    주기적 재수입   --format json 을 쓸 것

## 사라진 자원 찾기 — 최근 수집(last_seen)

수집에 없어진 자원이 계속 `사용중` 으로 남아 있으면 "껐는데 대장엔 살아 있음"이
된다. 자산 관리에서 실제로 위험한 상태다.

**자동으로 폐기 처리하지 않는다.** 부분 수입 때문이다 - `--source ec2-instances`
만 넣었는데 나머지를 "사라졌다"고 판정하면 멀쩡한 자원이 죽는다. 대신 수입이
확인한 자원에 시각을 남기고, 오래된 것을 사람이 보고 정한다.

    last_seen   수집(일괄 등록)에서 마지막으로 확인된 시각
                손으로 넣은 자원은 비어 있다

찾는 것은 필터로 한다.

    stale_days=30    30일 넘게 수집에서 안 보인 자원 (사라졌을 가능성)
    never_seen=true  한 번도 수집된 적 없는 자원 (= 손으로 넣은 것)

`stale_days` 는 한 번도 수집된 적 없는 자원을 **제외한다.** 손으로 넣은 자원은
"사라졌다"고 볼 근거가 없다.

폐기 표시는 **이미 있는 일괄 편집**으로 한다 - 필터로 걸러 전부 선택하고 상태를
`폐기예정` 으로 바꾸면 된다. 그래서 새 동작을 만들지 않았다.

`last_seen` 기록은 `update()` 로 한다. `save()` 를 돌리면 시각만 바뀐 변경 이력이
매 수입마다 쌓여 진짜 변경이 묻힌다. 실측 - 같은 자료를 다시 넣으면 변경 이력
증가가 0 이다.
...
<!-- verified: 2026-09-07 | how: 이미지 4.6.9-13-g862826fc0 로 배포 후
    ...
    최근 수집       수입 시 last_seen 기록, 수기 자원은 비어 있음
                    ...
                    동일 자료 재수입 시 변경 이력 증가 0 (이력 오염 없음)
    사람 칸 보존    재수입 후에도 tenant·monthly_cost·description 유지
                    같은 수입에서 기계 칸(status·instance_type)은 갱신됨
